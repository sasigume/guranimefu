...
const AnimeSingle = ({ anime }: Props) => {
  return (
    <Box>
      <Box as="h2" fontSize="2rem" fontWeight="bold">{anime.title_japanese} ({anime.title})</Box>
      <Box>データキャッシュ保持: {dayjs((anime.lastUpdateTime._seconds+anime.cacheTtlOfRanking)*1000).format('YYYY/MM/DD HH:mm:ss')}まで</Box>
      <Divider my={6} />
      <Box as="h3" mb={4} fontSize="1.7rem" fontWeight="bold">詳細情報</Box>
      <Flex direction={{ base: "column", md: "row" }}>
        ...
        <Box>
          ...
          <List mb={4}>
            ...
            <ListItem>{dayjs(anime.end_date).format('YYYY年MM月')}放送終了</ListItem>
          </List>
          <LinkChakra isExternal target="_blank" href={anime.url}><Button colorScheme="blue">MyAnimeListで詳細を見る</Button></LinkChakra>
        </Box>
      </Flex>
      <Divider my={6} />
      <Box as="h3" fontSize="1.7rem" fontWeight="bold">グラフ</Box>
